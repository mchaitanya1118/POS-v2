...
export default function CustomersPage() {
  ...
  const [loyaltyProfile, setLoyaltyProfile] = useState<{ points_balance: number; tier: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchLoyalty = async () => {
      try {
        const profile = await apiClient.get<any>(`/api/v1/loyalty/profile/${selectedCustomerId}`);
          ...
          <div className="flex-1 glass-panel rounded-3xl p-6 flex flex-col h-full overflow-hidden min-h-0 select-none">
            {activeCustomer ? (
              <div className="flex-1 flex flex-col h-full overflow-hidden min-h-0">
                ...
                <div className="grid grid-cols-4 gap-4 py-6 shrink-0 border-b border-slate-200 dark:border-slate-800">
                  ...
                  <div className="bg-slate-100/50 dark:bg-slate-800/40 p-4 rounded-2xl border border-indigo-500/20 bg-indigo-500/5">
                    <span className="text-[10px] uppercase font-bold tracking-wider text-indigo-400 block mb-1">Loyalty Points & Tier</span>
                    {loyaltyProfile ? (
                      <div className="flex items-center gap-1.5 mt-1.5">
                        <span className="text-xl font-extrabold text-indigo-500 tracking-tight">
                          {loyaltyProfile.points_balance} pts
                        </span>
                        <span className="text-[9px] uppercase font-black tracking-wider bg-indigo-500/10 text-indigo-550 px-1.5 py-0.5 rounded">
                          {loyaltyProfile.tier}
                        </span>
                      </div>
                    ) : (
                      <span className="text-xs font-bold text-slate-400 block mt-2">Inactive / Starter</span>
                    )}
                  </div>
                </div>
