...
export default function PosPage() {
      ...
      if (tableParam) {
        ...
        if (occupiedT?.runningOrderId) {
          ...
          if (activeOrd) {
            ...
            clearCart();
            setEditingOrder(activeOrd.id, activeOrd.orderNumber);
  ...
  const handlePlaceOrder = async (isDraft: boolean) => {
    ...
    try {
      const { editingOrderId, editingOrderNumber } = useCartStore.getState();

      const orderId = editingOrderId || (selectedTableId 
        ? (tables.find(t => t.id === selectedTableId)?.runningOrderId || `ord_${Date.now()}`)
        : `ord_${Date.now()}`);

      const generatedNum = editingOrderNumber || `INV-${Math.floor(100000 + Math.random() * 900000)}`;
  ...
  const handleEditOrder = (order: Order) => {
    ...
    setEditingOrder(order.id, order.orderNumber);
